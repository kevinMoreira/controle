...
function select($dataIni, $dataFinal){
    ...
    $totalSemana = "select round(sum(i.valorUnitario * i.qtde),2) from item_venda i
    inner join venda v on v.idvenda = i.idvenda
    where (DATE_FORMAT(v.dataVenda,'%m-%d-%Y') between DATE_FORMAT('$inicio','%m-%d-%Y') and DATE_FORMAT('$final','%m-%d-%Y')) and i.cancelado=0";
    ...
    for($time = $dataIni; $time <= $dataFinal; $time = strtotime('+1 day', $time)){
        ...
        $sql = "select round(sum(i.valorUnitario * i.qtde),2) from item_venda i
        inner join venda v on v.idvenda = i.idvenda
        where DATE_FORMAT(v.dataVenda,'%m-%d-%Y') = DATE_FORMAT('$data','%m-%d-%Y') and i.cancelado = 0";
